...
async def predict_fruit(
    ...
    if fruit_image.content_type not in ["image/jpeg", "image/png"]:
        raise HTTPException(status_code=400, detail="File harus JPG atau PNG")

    contents = await fruit_image.read()
    ...
    try:
        ...
        accuracy = float(np.max(proba))   # ambil nilai tertinggi
    ...
    return {
        "success": True,
        "filename": fruit_image.filename,
        "predicted_fruit": predicted,
